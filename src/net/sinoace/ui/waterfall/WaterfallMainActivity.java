...
import net.sinoace.ui.waterfall.WaterfallLazyScrollView.OnScrollListener;

public class WaterfallMainActivity extends Activity {
	...
	private HashMap<Integer, WaterfallFlowView> iviews;
	int scroll_height;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		context = this;
		iviews = new HashMap<Integer, WaterfallFlowView>();
	...
	private void InitLayout() {
		waterfall_scroll = (WaterfallLazyScrollView) findViewById(R.id.waterfall_scroll);
					...
					if (t > 2 * scroll_height) {// 超过两屏幕后
						...
						for (int k = 0; k < column_count; k++) {
							...
							if (pin_mark[k].get(Math.min(bottomIndex[k] + 1,
								...
								((WaterfallFlowView) waterfall_items.get(k).getChildAt(
							...
							if (pin_mark[k].get(topIndex[k]) < t - 2
								...
								int i1 = topIndex[k];
								topIndex[k]++;
								((WaterfallFlowView) localLinearLayout.getChildAt(i1))
										.recycle();
					...
					if (t > 2 * scroll_height) {// 超过两屏幕后
						for (int k = 0; k < column_count; k++) {
							...
							if (pin_mark[k].get(bottomIndex[k]) > t + 3
									* scroll_height) {
								((WaterfallFlowView) localLinearLayout
							...
							if (pin_mark[k].get(Math.max(topIndex[k] - 1, 0)) >= t
									- 2 * scroll_height) {
								((WaterfallFlowView) localLinearLayout.getChildAt(Math
		...
		handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case 1:
					...
					WaterfallFlowView v = (WaterfallFlowView) msg.obj;
					int w = msg.arg1;
					int h = msg.arg2;
					// Log.d("MainActivity",
					// String.format(
	...
	private void AddImage(String filename, int rowIndex, int id) {
		...
		WaterfallFlowView item = new WaterfallFlowView(context);
		...
		// 多线程参数
		WaterfallFlowTag param = new WaterfallFlowTag();
		param.setFlowId(id);
